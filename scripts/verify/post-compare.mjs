// The same frame with the post-processing stack off and on, for the comparison that is the only
// honest way to judge it.
//
// Usage: node scripts/verify/post-compare.mjs --from x,y,z --at x,y,z [--map last-resort]
//        [--out dir] [--size 1280x720] [--session name]
...
const out = resolve(root, opt('--out', 'build/review/post'));
const [width, height] = opt('--size', '1280x720').split('x').map(Number);
const session = opt('--session', `post-${process.pid}`);
...
try {
 ...
 for (const on of [false, true]) {
  evalJs(`(()=>{const g=window.__openSpeed;
   ...
   return 'ok'})()`);
  await sleep(500);
  const file = join(out, `${map}-post-${on ? 'on' : 'off'}.png`);
  ab('screenshot', file);
  console.log(`post ${on ? 'on ' : 'off'} -> ${file}`);
 }
} finally {
